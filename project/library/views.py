...
CLIENT_ID = settings.LIBRARY_CARD_CLIENT_ID
CLIENT_SECRET = settings.LIBRARY_CARD_CLIENT_SECRET
REDIRECT_URI = f"{HOST_BASE_URL}/{APP_NAME}/callback/"


@oauth_required(app_name=APP_NAME)
def index(request):
    """LIbrary homepage, requires OAuth authorisation (redirects to login if not yet authorised)."""

    # Get user data through calling internal API GET request.
    token = get_token(request)
    url = f"{HOST_BASE_URL}/api/me/"
    headers = {'Authorization': f"Bearer {token}", 'context': 'library'}
...
def callback(request):
    ...
    next_url = request.session.pop('oauth_next', 'library:index')
