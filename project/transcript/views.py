...
def index(request):
    """Transcript service, requires OAuth authorisation (redirects to login if not yet authorised)."""

    # Get user data through calling internal API GET request.
    token = get_token(request)
    url = f"{HOST_BASE_URL}/api/me/"
    headers = {'Authorization': f"Bearer {token}", 'context': 'transcript'}
    ...
    if api_response.status_code == 200:
        ...
        context = {
            ...
            'institutional_id': data.get('institutional_id', None),
            'modules': [a for a in affiliations if a['affiliation_type'] == 'MOD'],
            }
        return render(request, 'transcript/index.html', context)
...
def view_roster(request, roster_type, affiliation_id):
    ...
    return render(request, 'transcript/partials/roster_list.html', context)