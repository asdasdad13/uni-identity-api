...
CLIENT_ID = settings.STAFF_DIRECTORY_CLIENT_ID
CLIENT_SECRET = settings.STAFF_DIRECTORY_CLIENT_SECRET
REDIRECT_URI = f"{HOST_BASE_URL}/{APP_NAME}/callback/"


@oauth_required(app_name=APP_NAME)
def index(request):
    """Staff Directory homepage, requires OAuth authorisation (redirects to login if not yet authorised)."""

    # Get user data through calling internal API GET request.
    token = get_token(request)
    url = f"{HOST_BASE_URL}/api/me/"
    headers = {'Authorization': f"Bearer {token}", 'context': 'staff'}
    ...
    if api_response.status_code == 200:
        ...
        context = {
            ...
            'departments': [a for a in affiliations if a['affiliation_type'] == 'DEPT'],
            }
...
def login(request):
    ...
    params = {
        ...
        'scope': 'openid profile identity affiliations:departments',
...
def callback(request):
    ...
    request.session['departments'] = userinfo.get('affiliations:departments', [])
    ...
    next_url = request.session.pop('oauth_next', 'staff:index')
...
def view_roster(request, roster_type, affiliation_id):
    ...
    # Matches the API structure: /api/roster/department/CS101/
    ...
    return render(request, 'staff/partials/roster_list.html', context)